...
import { initWorkspaceUi, prepareWorkspaceUi } from './workspace-ui.js';
declare const __BRAND_NAME__: string;

// Align body classes with the early html.workspace boot script (theme-boot partial).
prepareWorkspaceUi();
...
const init = async () => {
  ...
  if (dom.toolGrid) {
      ...
      category.tools.forEach((tool) => {
        ...
        if (tool.subtitle) {
          ...
          toolSubtitle.textContent = toolKey
            ? t(`${toolKey}.subtitle`)
            : tool.subtitle;
          copy.appendChild(toolSubtitle);
        }

        toolCard.append(icon, copy);
    ...
    searchResultsContainer.className = 'hidden col-span-full';
    ...
    searchBar.addEventListener('input', () => {
      ...
      if (!searchTerm) {
        ...
        categoryGroups.forEach((group) => {
          const filtered = group.classList.contains('is-filtered-out');
          (group as HTMLElement).style.display = filtered ? 'none' : '';
      ...
      if (allTools.length === 0) {
        ...
        empty.className = 'workspace-empty-search';

        const title = document.createElement('strong');
        title.textContent = `No tools match “${searchTerm}”`;

        const hint = document.createElement('p');
        hint.textContent =
          'Try a broader term like merge, compress, sign, or convert.';
  ...
  if (shortcutsTabBtn && preferencesTabBtn) {
    const activateSettingsTab = (tab: 'shortcuts' | 'preferences') => {
      const showShortcuts = tab === 'shortcuts';
      shortcutsTabBtn.classList.toggle('is-active', showShortcuts);
      preferencesTabBtn.classList.toggle('is-active', !showShortcuts);
      shortcutsTabBtn.setAttribute(
        'aria-selected',
        showShortcuts ? 'true' : 'false'
      );
      preferencesTabBtn.setAttribute(
        'aria-selected',
        showShortcuts ? 'false' : 'true'
      );
      // Clear legacy utility classes if present
      shortcutsTabBtn.classList.remove(
        'bg-indigo-600',
        'text-white',
        'text-gray-300'
      );
      preferencesTabBtn.classList.remove(
        'bg-indigo-600',
        'text-white',
        'text-gray-300'
      );

      shortcutsTabContent?.classList.toggle('hidden', !showShortcuts);
      preferencesTabContent?.classList.toggle('hidden', showShortcuts);
      if (shortcutsTabContent) {
        shortcutsTabContent.hidden = !showShortcuts;
      }
      if (preferencesTabContent) {
        preferencesTabContent.hidden = showShortcuts;
      }
      shortcutsTabFooter?.classList.toggle('hidden', !showShortcuts);
      preferencesTabFooter?.classList.toggle('hidden', showShortcuts);
      resetShortcutsBtn?.classList.toggle('hidden', !showShortcuts);
    };

    shortcutsTabBtn.addEventListener('click', () =>
      activateSettingsTab('shortcuts')
    );
    preferencesTabBtn.addEventListener('click', () =>
      activateSettingsTab('preferences')
    );

    // Default: Preferences (better default on mobile and for most users)
    activateSettingsTab('preferences');
  }
  ...
  if (dom.shortcutsModal) {
    ...
    document.addEventListener('keydown', (e) => {
      if (
        e.key === 'Escape' &&
        dom.shortcutsModal &&
        !dom.shortcutsModal.classList.contains('hidden')
      ) {
        closeSettingsModal();
      }
    });
  ...
  function renderShortcutsList() {
    ...
    shortcutCategories.forEach((category) => {
      ...
      header.className = 'settings-section-label';
      const categoryKey = categoryTranslationKeys[category.name];
      header.textContent = categoryKey ? t(categoryKey) : category.name;
      section.appendChild(header);

      const itemsContainer = document.createElement('div');
      itemsContainer.className = 'settings-shortcut-group';
      ...
      category.tools.forEach((tool) => {
        ...
        item.className = 'shortcut-item settings-shortcut-row';

        const left = document.createElement('div');
        left.className = 'settings-shortcut-meta';

        const icon = document.createElement('i');
        if (tool.icon.startsWith('ph-')) {
          icon.className = `ph ${tool.icon} settings-shortcut-icon`;
        } else {
        ...
        name.className = 'settings-shortcut-name';
        const toolKey = toolTranslationKeys[tool.name];
        ...
        right.className = 'settings-shortcut-control relative';

        const input = document.createElement('input');
        input.type = 'text';
        input.className = 'shortcut-input settings-shortcut-input';
