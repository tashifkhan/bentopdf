...
const resetState = async () => {
  ...
  if (fileModeBtn && pageModeBtn && filePanel && pagePanel) {
    fileModeBtn.classList.add('is-active');
    fileModeBtn.setAttribute('aria-selected', 'true');
    pageModeBtn.classList.remove('is-active');
    pageModeBtn.setAttribute('aria-selected', 'false');
...
export async function refreshMergeUI() {
  ...
  (state.files as File[]).forEach((f, index) => {
    ...
    mainDiv.className = 'tool-file-top';

    const dragHandle = document.createElement('div');
    dragHandle.className = 'tool-file-handle drag-handle';
    ...
    mainDiv.append(dragHandle, nameSpan);

    const rangeDiv = document.createElement('div');
    rangeDiv.className = 'tool-file-range';
    ...
    label.textContent = `Pages · ${pageCount} total`;

    const input = document.createElement('input');
    input.type = 'text';
    input.id = `range-${safeFileName}`;
    input.placeholder = 'All pages (or e.g. 1-3, 5)';
    ...
    deleteBtn.className = 'tool-file-remove';
  ...
  const setModeActive = (mode: 'file' | 'page') => {
    const fileActive = mode === 'file';
    newFileModeBtn.classList.toggle('is-active', fileActive);
    newPageModeBtn.classList.toggle('is-active', !fileActive);
    newFileModeBtn.setAttribute('aria-selected', fileActive ? 'true' : 'false');
    newPageModeBtn.setAttribute('aria-selected', fileActive ? 'false' : 'true');
    // Drop legacy utility classes if present from older markup
    newFileModeBtn.classList.remove(
      'bg-indigo-600',
      'text-white',
      'bg-gray-700',
      'text-gray-300'
    );
    newPageModeBtn.classList.remove(
      'bg-indigo-600',
      'text-white',
      'bg-gray-700',
      'text-gray-300'
