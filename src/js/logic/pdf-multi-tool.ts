...
/** Open the shared multi-file picker (never relies on display:none inputs). */
function openPdfFilePicker() {
  ...
  // Absolute fallback if the DOM input is missing
  const ephemeral = document.createElement('input');
  ephemeral.type = 'file';
  ephemeral.accept =
    'application/pdf,image/*,.heic,.heif,.jpg,.jpeg,.png,.bmp,.gif,.tiff,.tif,.webp,.svg';
  ephemeral.multiple = true;
  ...
  ephemeral.addEventListener('change', () => {
    void handlePdfUpload({ target: ephemeral } as unknown as Event);
    ephemeral.remove();
  });
  ephemeral.click();
}

/** Wire file pick / change first so Select Files works even if later init throws. */
function wireFilePickers() {
  ...
  if (insertInput && !insertInput.dataset.wired) {
    insertInput.dataset.wired = '1';
    insertInput.addEventListener('change', handleInsertPdf);
  }

  // Label already opens the picker via `for`; stop bubble so upload-area
  // doesn't double-fire. Keyboard activation for accessibility.
  const selectBtn = document.getElementById('pdf-file-input-select-btn');
  if (selectBtn && !selectBtn.dataset.wired) {
    selectBtn.dataset.wired = '1';
    selectBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      // If it's a <label for=…>, the browser opens the picker natively.
      // Only force-click when the control is a plain button.
  ...
  if (uploadArea && !uploadArea.dataset.wired) {
    uploadArea.dataset.wired = '1';
    uploadArea.addEventListener('click', (e) => {
      // Ignore clicks on the label / interactive controls inside
      const target = e.target as HTMLElement | null;
      if (target?.closest('label, button, a, input, select, textarea')) return;
      openPdfFilePicker();
    });
  }

  const uploadBtn = document.getElementById('upload-pdfs-btn');
...
function bootMultiTool() {
  console.log('PDF Multi Tool: boot');
  // Critical path first — file select must work even if icons/shortcuts fail
  try {
    wireFilePickers();
    document.getElementById('upload-area')?.classList.remove('hidden');
  } catch (e) {
    console.error('PDF Multi Tool: file picker wiring failed', e);
  }

  try {
    initializeTool();
  } catch (e) {
    console.error('PDF Multi Tool: initializeTool failed', e);
    // Retry file wiring in case initializeTool partially clobbered the DOM
    try {
      wireFilePickers();
    } catch {
      /* ignore */
...
function initializeTool() {
  ...
  try {
    ...
    console.warn('PDF Multi Tool: createIcons failed', e);
  }

  try {
    initializeGlobalShortcuts();
  } catch (e) {
    console.warn('PDF Multi Tool: shortcuts init failed', e);
  }

  // Mobile dock buttons proxy to the real toolbar controls (same IDs / handlers).
  document.querySelectorAll<HTMLElement>('[data-mt-proxy]').forEach((btn) => {
    btn.addEventListener('click', (event) => {
      event.preventDefault();
      const targetId = btn.dataset.mtProxy;
      if (!targetId) return;
  ...
  // File pickers already wired in wireFilePickers(); keep a second pass for safety
  wireFilePickers();
