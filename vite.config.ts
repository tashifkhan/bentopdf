...
/** Prefix internal rewrite paths with Vite `base` (e.g. `/t`) in dev. */
function withBasePath(absolutePath: string): string {
  const basePath = getBasePath();
  if (!basePath || basePath === '/') return absolutePath;
  if (absolutePath.startsWith(basePath + '/') || absolutePath === basePath) {
    return absolutePath;
  }
  return `${basePath}${absolutePath.startsWith('/') ? '' : '/'}${absolutePath}`;
}
      ...
      if (pageName && PAGES.has(pageName)) {
        if (isDev) {
          ...
          if (fs.existsSync(srcPath)) {
            req.url = withBasePath(`/src/pages/${pageName}.html`) + q;
          } else {
            req.url = withBasePath(`/${pageName}.html`) + q;
          }
        } else {
          ...
          if (fs.existsSync(langPagePath)) {
            req.url = withBasePath(`/${lang}/${pageName}.html`) + q;
          } else {
            req.url = withBasePath(`/${pageName}.html`) + q;
          }
        }
        ...
        if (isDev) {
          req.url = withBasePath(`/${cleanPath}.html`) + q;
        } else {
          const langPagePath = resolve(
            __dirname,
            'dist',
          ...
          if (fs.existsSync(langPagePath)) {
            req.url = withBasePath(`/${lang}/${cleanPath}.html`) + q;
          } else {
            req.url = withBasePath(`/${cleanPath}.html`) + q;
          }
        }
...
function themeBootPlugin(): Plugin {
  // Inject the FOUC boot snippet into every HTML <head> so tool pages
  // never paint the old indigo/gray shell before JS runs.
  const bootPath = resolve(__dirname, 'src/partials/theme-boot.html');
  let bootHtml = '';
  try {
    bootHtml = fs.readFileSync(bootPath, 'utf8');
  } catch {
    bootHtml = '';
  }
  ...
  return {
    ...
    transformIndexHtml: {
      order: 'pre',
      handler(html) {
        if (!bootHtml) return html;
        if (html.includes('id="ws-boot-css"') || html.includes('data-ws-boot')) {
          return html;
        }
        if (html.includes('</head>')) {
          return html.replace('</head>', `${bootHtml}\n</head>`);
        }
        return html;
      },
    },
...
function languageRouterPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      // In simple mode, serve the redesigned home at `/` during dev.
      if (process.env.SIMPLE_MODE === 'true') {
        server.middlewares.use((req, _res, next) => {
          ...
          if (
            ...
            req.url = url.replace(pathOnly, '/simple-index.html');
          }
          next();
        });
      }
