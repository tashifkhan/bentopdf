    ...
    return (
      <label className="flex items-start gap-2.5" htmlFor={id}>
        <input
          ...
          className="mt-0.5 size-4 rounded border-border accent-[var(--brand,#6366f1)]"
        />
        <span>
  ...
  return (
    <label className="block" htmlFor={id}>
      <span className="mb-1.5 flex items-baseline justify-between gap-2">
        ...
        {field.type === 'range' ? (
          <span className="text-[11px] tabular-nums text-ink-4">{value}</span>
        ) : null}
      </span>
      ...
      {field.type === 'select' ? (
        ...
        <textarea
          ...
          rows={8}
          className="w-full rounded-xl border border-border bg-card px-3 py-2 font-mono text-sm text-foreground outline-none focus:border-brand"
        />
      ) : field.type === 'range' ? (
        <input
          id={id}
          ...
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="w-full accent-[var(--brand,#6366f1)]"
        />
      ) : field.type === 'color' ? (
        ...
        <span className="block">
          <input
            ...
            className="block w-full cursor-pointer rounded-xl border border-border bg-card px-3 py-2 text-sm text-muted-foreground file:mr-3 file:rounded-lg file:border-0 file:bg-secondary file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-foreground"
          />
          {files.length > 0 ? (
