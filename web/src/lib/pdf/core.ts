...
export async function deletePages(
  ...
  if (remove.size === 0)
    throw new Error('Specify pages to delete (e.g. 2,4-6)');
...
export async function addBlankPage(
  ...
  const ref =
    count > 0 ? doc.getPage(Math.min(Math.max(afterPage, 1), count) - 1) : null;
  const size = ref
...
export async function compressPdfRaster(
  ...
  quality = 0.65,
  scale = 1.25,
  opts: { greyscale?: boolean; stripMetadata?: boolean } = {}
): Promise<Uint8Array> {
  const pdfjs = await getPdfjs();
  const data = await file.arrayBuffer();
  const pdf = await pdfjs.getDocument({ data: data.slice(0) }).promise;
...
export async function pageNumbersPdf(
  ...
  doc.getPages().forEach((page: PDFPage, i: number) => {
    ...
    const label = format === 'n/N' ? `${i + 1} / ${total}` : String(i + 1);
...
/** Extract only the even- or odd-numbered pages (1-based). */
export async function splitByParity(
  file: File,
  parity: 'even' | 'odd'
): Promise<Uint8Array> {
  const doc = await loadPdf(file);
  const indices = Array.from(
    ...
    (_, i) => i
  ).filter((i) => ((i + 1) % 2 === 0) === (parity === 'even'));
  if (indices.length === 0) throw new Error(`No ${parity} pages found`);
  const out = await copyPagesToNew(doc, indices);
  return out.save();
}

/** Insert one or more blank pages at a position, sized to match the document. */
export async function addBlankPages(
  file: File,
  position: 'start' | 'end' | 'after' | 'every',
  afterPage: number,
  count: number
): Promise<Uint8Array> {
  const src = await loadPdf(file);
  const total = src.getPageCount();
  const ref = total > 0 ? src.getPage(0) : null;
  ...
  const out = await PDFDocument.create();
  const addBlanks = () => {
    for (let n = 0; n < count; n++) out.addPage(size);
  };
  ...
  for (let i = 0; i < total; i++) {
    ...
    out.addPage(copied);
    if (position === 'every') addBlanks();
    if (position === 'after' && i === Math.max(0, afterPage - 1)) addBlanks();
  }
